...
> 代码得规范健壮 **Typescript** 也是很好的工具，加入类型校验，在写代码过程中就可以很好的预检查。为了减少在语言上的门槛，在代码上。本项目先不添加TypeScript。后期更新项目改造 **Typescript**
...
## 1：编程规范一：代码检测工具 ESLint

Vue-Cli 创建项目已经把这部分集成到项目中，EsLint，代码格式化等都可以选。所以接下来是手动安装和配置。
...
![image-20210904185336318.png](./images/image-20210904185336318.png)
...
## 3：ESLint 与 Prettier 配合解决代码格式问题
...
1. 在 `VSCode` 中安装 `prettier` 插件（搜索 `prettier`），这个插件可以帮助我们在配置 `prettier` 的时候获得提示
   ...
   ![image-20210904195026475.png](./images/image-20210904195026475.png)
...
4. 打开 `VSCode` 《设置面板》
   ![image-20210904200638072.png](./images/image-20210904200638072.png)

5. 在设置中，搜索 `save` ，勾选 `Format On Save`
   ![image-20210904200738067.png](./images/image-20210904200738067.png)
...
![image-20210904201057594.png](./images/image-20210904201057594.png)
...
![image-20210904203051754.png](./images/image-20210904203051754.png)
...
![image-20210904205519762.png](./images/image-20210904205519762.png)

我想大家看到这样的一个提交描述之后，心里的感觉应该和我一样是崩溃的！要是每次都这么写，写到猴年马月了！

如果你有这样的困惑，接下来我们将一起解决这个问题！

## 5：Commitizen 助你规范化提交代码
...
## 7：使用 husky + commitlint 检查提交描述是否符合规范要求
...
![image-20210710121456416.png](./images/image-20210710121456416.png)
...
2. 启动 `hooks` ， 生成 `.husky` 文件夹
   ...
   ```
    ...
    ![image-20210906202034156.png](./images/image-20210906202034156.png)
...
3. 在 `package.json` 中生成 `prepare` 指令（ **需要 npm > 7.0 版本** ）
   ...
   ```
    ...
    ![image-20210906202128323.png](./images/image-20210906202128323.png)
...
5. 执行成功，提示
![image-20210710120053221.png](./images/image-20210710120053221.png)
...
2. 该操作会生成对应文件 `pre-commit`：
    ![image-20210906204043915.png](./images/image-20210906204043915.png)
...
## 10：关于 `vetur` 检测 `template` 的单一根元素的问题（图文）
